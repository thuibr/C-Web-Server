...
struct cache_entry *alloc_entry(char *path, char *content_type, void *content, int content_length)
{
    ...
    // Copy path into entry
    size_t size = sizeof(*path) * strlen(path) + 1;
    ce->path = calloc(1, size);
    ce->path = strncpy(ce->path, path, size);

    // Copy content_type into entry
    size = sizeof(*content_type) * strlen(content_type) + 1;
    ce->content_type = calloc(1, size);
    ce->content_type = strncpy(ce->content_type, content_type, size);

    ce->content_length = content_length;

    // Copy content into entry
    size = content_length;
    ce->content = calloc(1, size);
    ce->content = memcpy(ce->content, content, size);
...
void cache_put(struct cache *cache, char *path, char *content_type, void *content, int content_length)
{
    ...
    // Evict the tail if cache is maxed out
